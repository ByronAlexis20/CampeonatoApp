...
public class JugadorLista {
	...
	public void buscar(){
		...
		if(jugadores.size() == 0) {
			...
			for(int i=0; i<jugadores.size(); i++) {
				if(jugadores.get(i).getGenero().equals("F")) {
					jugadores.get(i).setGenero("FEMENINO");					
				}else if (jugadores.get(i).getGenero().equals("M")) {
					jugadores.get(i).setGenero("MASCULINO");
				}
			}			
	...
	public void nuevo(){
		...
		Window ventanaCargar = (Window) Executions.createComponents("/Mantenimiento/jugadores/jugadorEditar.zul", null, null);
