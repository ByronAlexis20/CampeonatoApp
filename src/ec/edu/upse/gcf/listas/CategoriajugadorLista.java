...
public class CategoriajugadorLista {
	...
	public void buscar(){
		...
		if(categoriajugador.size() == 0) {
			...
			for(int i=0; i<categoriajugador.size(); i++) {
				if(categoriajugador.get(i).getCategoria().getTipo().equals("F")) {
					categoriajugador.get(i).getCategoria().setTipo("FEMENINO");					
				}else if (categoriajugador.get(i).getCategoria().getTipo().equals("M")) {
					categoriajugador.get(i).getCategoria().setTipo("MASCULINO");
				}
			}			
