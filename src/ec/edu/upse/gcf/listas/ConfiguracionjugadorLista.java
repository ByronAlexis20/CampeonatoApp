...
public class ConfiguracionjugadorLista {
	...
	public void marcarLista() {
		Date fecha = new Date();
		if(!confjugador.isEmpty()) {			
			for(Configuracionjugador config : configuracionjugadorDao.getListaConfiguracionjugadores()) {
				if(fecha.compareTo(config.getFechaInicio()) == 1
					&& fecha.compareTo(config.getFechaFin()) == 1) {
					config.setEstadoconf("CULMINADO");												
				}
			}
		}
	}
