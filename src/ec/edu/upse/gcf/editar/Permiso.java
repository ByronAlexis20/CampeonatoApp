...
public class Permiso {
	...
	public void listaropciones() {
		...
		if(perfilseleccionado != null) {
			//listado de las opciones que tiene asignado el perfil
			...
			if (!accesos.isEmpty()) {
				for (Opcionperfil opcionperfil : accesos) {
					...
					opc = opcionperfil.getOpcion();
					//armar el titulo del menu con los padres
					while(opc.getIdOpcionPadre() != null) {
						menu.add(opc.getTitulo());
						//System.out.println("Menu padre: " + opc.getTitulo());
						opc = opcionDao.getOpcionId(opc.getIdOpcionPadre());
					}
					menu.add(opc.getTitulo());
					String menuTitulo = "";
					for(int i = menu.size() - 1 ; i >= 0 ; i --) {
						menuTitulo = menuTitulo + "/" + menu.get(i);
					}
					opcionperfil.getOpcion().setTituloMostrar(menuTitulo);
					opcionesAsignadas.add(opcionperfil.getOpcion());
				}
			}
			//listado de opciones que se encuentran disponibles
			...
			if (!opciones.isEmpty()) {
				for (Opcion objeto : opciones) {
					...
					objeto.setTituloMostrar(menuTitulo);
