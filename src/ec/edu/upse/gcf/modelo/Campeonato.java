...
@Entity
@NamedQueries({
	@NamedQuery(name="Campeonato.findAll", query="SELECT c FROM Campeonato c WHERE c.estadocamp = 'D' ORDER BY c.idCampeonato DESC "),
	@NamedQuery(name="Campeonatos.buscarPorPatron", 
	query="SELECT c FROM Campeonato c WHERE LOWER(c.nombreC) LIKE LOWER(:patron) ORDER BY c.idCampeonato DESC")
})
