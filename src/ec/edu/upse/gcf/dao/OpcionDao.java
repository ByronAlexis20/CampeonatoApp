...
public class OpcionDao extends ClaseDAO {
	...
	public Opcion getOpcionId(Integer idOpcion) {
		Opcion resultado;  
		Query query = getEntityManager().createNamedQuery("Opcion.opcionMenu");
		query.setHint("javax.persistence.cache.storeMode", "REFRESH");
		query.setParameter("idOpcion", idOpcion);
		resultado = (Opcion) query.getSingleResult();
		return resultado;
	}
	public List<Opcion> getMenuHijoPadre(Integer idPadre) {
		List<Opcion> resultado;  
		Query query = getEntityManager().createNamedQuery("Opcion.menuHijoPadre");
		query.setHint("javax.persistence.cache.storeMode", "REFRESH");
