...
public class OpcionPerfilDAO extends ClaseDAO{
	...
	public List<Opcionperfil> getMenuPerfil(Integer id) {
		List<Opcionperfil> resultado; 
		try {			
			Query query = getEntityManager().createNamedQuery("Opcionperfil.BuscarOpcionMenuPerfilPadre");
			query.setHint("javax.persistence.cache.storeMode", "REFRESH");
			query.setParameter("perfil", id);
			resultado = (List<Opcionperfil>) query.getResultList();
			return resultado;
	...
	public List<Opcionperfil> getMenuHijos(Integer idPerfil) {
		List<Opcionperfil> resultado; 
		try {			
			Query query = getEntityManager().createNamedQuery("Opcionperfil.opcionesHijos");
			query.setHint("javax.persistence.cache.storeMode", "REFRESH");
